...
window.onload = async () => {
  filterBtn.addEventListener("click", async () => {
    ...
    if (showOnlySouthernFinlandCenters.checked) {
      console.log(
        "Showing rescue centers and their contacts persons in Southern Finland"
      );
    ...
    if (showOnlyEasternFinlandCenters.checked) {
      console.log(
        "Showing rescue centers and their contacts persons in Eastern Finland"
      );
    ...
    if (showOnlyProvinceOfOuluCenters.checked) {
      console.log(
        "Showing rescue centers and their contacts persons in Province of Oulu"
      );
...
function createRescueCenterRow(center, id) {
  ...
  let centerNameText = addText(
    center.rescue_center_id,
    center.rescue_center_name
  );
  centerNameColumn.appendChild(centerNameText);
  let centerLocationColumn = document.createElement("td");
  let centerLocationText = addText(
    "location#" + center.rescue_center_id,
  ...
  let contactPersonNameText = addText(
    "name#" + contact_person_id,
    center.contact_person
  );
  let contactPersonEmailText = addText(
    "email#" + contact_person_id,
    center.email
  );
  let contactPersonPhoneText = addText(
    "phone#" + contact_person_id,
    center.phone
  );
